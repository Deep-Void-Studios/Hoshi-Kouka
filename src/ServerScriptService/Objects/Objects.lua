...
local resources = game:GetService("ServerStorage").Objects
...
module.place = function(obj, cframe)
	...
	if not directory:FindFirstChild(object:GetAttribute("Type")) then
		local folder = Instance.new("Folder", directory)
...
module.placeNC = function(obj, cframe)
	...
	for i, group in pairs(typegroups) do
		for i, ty in pairs(group) do
...
module.randomDecoration = function(types)
	...
	for i, obj in pairs(resources:GetChildren()) do
