...
local equipment = ClientComm:GetProperty(data.Equipment.Id)
...
local function getSignal(item)
	return ClientComm:GetSignal(item.Id)
end
...
local function setSlot(slot)
	if slot == "BuildMode" or slot == "Pickaxe" then return end
...
UIS.InputBegan:Connect(function(input, ignored)
	if not ignored and player.Character and
		(gui.Enabled or input.KeyCode == Enum.KeyCode.Three) then
...
equipment.Updated:Connect(function()
...
for _, button in pairs(container:GetChildren()) do
	if button:IsA("TextButton") then
		button.Activated:Connect(function()
			local slot = equipment[button.Name]
